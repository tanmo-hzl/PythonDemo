...
class RequestBodyOrderFlow(object):
    ...
    def get_active_listing(data, variation: Optional):
        if variation == "False":
            ...
            items = []
            for item in data:
                if item.get("status") == "Active" and item.get("inventory") > 0:
                    items.append(item)
            return items
    ...
    def get_buyer_return_order_body(self, buyer_order_detail):
        path = os.path.join(self.root_path, "CaseData", "upload-img-data.json")
        ...
        sub_orders = buyer_order_detail["data"]["subOrders"][0]
        shipment = sub_orders["shipments"][0]
        order_lines_list = sub_orders["orderLines"]
        return_order_lines = []
        ...
        body = {
            ...
            "subReturnOrders": [
                {
                    "orderNumber": sub_orders["orderNumber"],
                    "shippingLabelsExpected": 1,
                    "channel": 2,
                    "address": {
                        "firstName": shipment["firstName"],
                        "lastName": shipment["firstName"],
                    ...
                    #     [
                    #     {
                    #         "orderItemId": sub_order["orderLines"][0]["orderItemId"],
                    #         "quantity": sub_order["orderLines"][0]["quantity"],
                    #         "afterSalesReason": after_sales_comment,
                    #         "afterSalesComment": "I don't Know what happen about the item",
                    #         "userUploadMedia": user_upload_media,
                    #         "itemCondition": item_condition[0],
