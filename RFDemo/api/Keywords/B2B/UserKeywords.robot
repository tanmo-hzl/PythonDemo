...
Michaels User Get Public Key - GET
    ...
    ${res}    Send Get Request Without Token   ${URL-B2B-MAP}    ${MICHAELS_GET_PUBLIC_KEY}     ${null}
...
Michaels User Sign In Secure - POST
    ...
    ${res}    Send Post Request Without Token    ${URL-B2B-MAP}    ${MICHAELS_SIGN_IN_SECURE}    ${body}
...
Get Michaels User Roles - GET
    ...
    ${res}     Send Post Request    ${URL-B2B-MAP}    ${GET_USER_ROLE}    ${NULL}    ${headers}

Get Michaels User Profile - GET
    ${headers}    Set Variable    ${Headers_Michaels_Get}
    ${headers}    Add Admin Token To Headers - B2B - Michaels    ${headers}
    ${res}     Send Get Request    ${URL-B2B-MAP}    ${GET_USER_PROFILE}    ${NULL}    ${headers}
...
Change Michaels User Profile - PATCH
    ...
    ${res}     Send Patch Request    ${URL-B2B-MAP}    ${GET_USER_PROFILE}    ${body}    ${headers}
    ${Michaels_User_Profile}    Get Json Value    ${res.json()}    data
...
Update Michaels User Password - PATCH
    ...
    ${res}     Send Patch Request    ${URL-B2B-MAP}    ${UPDATE_USER_PWD}    ${body}    ${headers}

Michaels User Sign Out - POST
    ${headers}    Set Variable    ${Headers_Michaels_Post}
    ${headers}    Add Admin Token To Headers - B2B - Michaels    ${headers}
    ${res}     Send Post Request    ${URL-B2B-MAP}    ${MICHAELS_USER_SIGN_OUT}    ${NULL}    ${headers}

Get User User Approval Permissions - GET
    ${res}     Send Get Request    ${URL-B2B-MAP}    ${USER_APPROVAL_PERMISSIONS}    ${NULL}    ${Headers_Michaels_Get}
