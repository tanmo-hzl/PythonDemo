...
Add User Payment Options - POST
    [Documentation]   add payment method
    [Arguments]  ${card_number}  ${cvv}
    ${body}    Get Create Payment Option Body  ${card_number}  ${cvv}
    Send Post Request - Params And Json  ${URL-B2B}  /users/${B2B_User_Id}/bank-cards   ${null}  ${body}  ${Headers_User_Post}  201

Delete User Payment Options - DELETE
    ${bank_card_id}  Get Json Value  ${User_Payment_Info}   privatePaymentMethodList  paymentMethodId
    IF  ${bank_card_id}
        Send Delete Request - Params   ${URL-B2B}    /users/${B2B_User_Id}/bank-cards/${bank_card_id}  ${null}  ${Headers_User_Post}  204
    ELSE
        log  there is no private bank_card!
        Skip
    END
...
#    ${private_bank_list}    Get json value    ${User_Payment_Info}    privatePaymentMethodList
#    ${org_bank_list}    Set Variable IF    '${Len_bank}'!='0'    ${private_bank_list}
...
Loop Delete Organization Payment Methods - DELETE
    ...
    FOR    ${index}    IN RANGE    ${quantity}
        ...
        run keyword if    '${expirationStatus}'=='ACTIVE'    Delete Organization Payment Methods - DELETE   ${org_id}    ${id}
    END

Delete Organization Payment Methods - DELETE
    [Arguments]    ${org_id}    ${id}
