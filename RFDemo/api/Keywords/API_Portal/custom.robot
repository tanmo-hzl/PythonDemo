...
Initial Env Data - API-Portal
    [Arguments]    ${config_name}=configAPIPotal.ini
...
Send Get Request By Invalid API
    [Arguments]     ${url}     ${path}=    ${data}=     ${expected_status}=200
    ${headers}      Set Invalid Headers
    Create Session      alias       url=${url}      headers=${headers}      verify=${true}
    ${response}     GET On Session   alias     ${path}     params=${data}    verify=${True}     expected_status=${expected_status}
	[Return]    ${response}
...
Set Invalid Headers
    ${headers}    Create Dictionary     Content-Type=text/html      User-Agent=Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:94.0) Gecko/20100101 Firefox/94.0
    [Return]    ${headers}
...
Send Post Request
	...
	${headers}      Set Get Headers
	Create Session      alias     url=${url}      headers=${headers}    verify=${True}
	${response}    POST On Session    alias     ${path}     json=${data}    verify=${True}     expected_status=${expected_status}
	[Return]    ${response}
...
Send Query Post Request
	...
	${headers}      Set Get Headers
	Create Session      alias     url=${url}      headers=${headers}    verify=${True}
	${response}    POST On Session    alias     ${path}     params=${data}    verify=${True}       expected_status=${expected_status}
	Log     ${response.text}
	[Return]    ${response}
...
Send Put Request
	...
	${headers}      Set Get Headers
	Create Session      alias     url=${url}      headers=${headers}    verify=${True}
	${response}    PUT On Session   alias     ${path}     json=${data}    verify=${True}       expected_status=${expected_status}
	[Return]    ${response}
