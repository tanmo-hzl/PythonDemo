...
Get Order List Query - GET
    ...
    ${res}    Send Get Request    ${URL-MIK-MDA}   ${dev-order-query}    ${body}    ${get_headers}
...
Get Order Detail By Order Number - GET
    ...
    ${res}    Send Get Request    ${URL-MIK-MDA}   ${dev-order-detail}${order_number}   ${body}    ${get_headers}

Order Confirm - POST
    ${order_number}    Get Json Value    ${order_list[0]}    orderBillingAddresses     orderNumber
    ${order_number}    Create List    ${order_number}
    ${body}    Get Order Confirm Body    ${order_number}
    ${res}    Send Post Request    ${URL-MIK-MDA}   ${dev-order-confirm}   ${body}    ${post_headers}

Order Shipment - POST
    ${body}    Get Order Shipped Body    ${order_list[0]}
    ${res}    Send Post Request    ${URL-MIK-MDA}   ${dev-order-shpped}   ${body}    ${post_headers}

Order Cancel - POST
    ${body}    Get Order Cancel Body    ${order_list[0]}
    ${res}    Send Post Request    ${URL-MIK-MDA}   ${dev-order-cancel}   ${body}    ${post_headers}

Get Return Order Query - GET
    [Arguments]    ${status}=${null}
    ${body}    Get Return Order Query Body    ${status}
    ${res}    Send Get Request    ${URL-MIK-MDA}   ${dev-return-order-list}    ${body}    ${get_headers}
    ${page_data}    Get Json Value    ${res.json()}    data    pageData
...
Get Return Order Detail By Return Order Number
    ...
    ${res}    Send Get Request    ${URL-MIK-MDA}   ${dev-return-order-by-return-order-number}/${return_order_id}    ${null}    ${get_headers}

Get Return Order Detail By Order Number
    ${order_number}    Get Json Value    ${return_order_list[0]}    orderNumber
    ${res}    Send Get Request    ${URL-MIK-MDA}   ${dev-return-order-by-return-order-number}/${order_number}    ${null}    ${get_headers}

Return Order Approval
    ${body}    Get Return Order Approve Body    ${return_order_list[0]}
    ${res}    Send Post Request    ${URL-MIK-MDA}   ${dev-return-order-approval}    ${body}    ${post_headers}

Return Order Reject
    ${body}    Get Return Order Reject Body    ${return_order_list[0]}
    ${res}    Send Post Request    ${URL-MIK-MDA}   ${dev-return-order-reject}    ${body}    ${post_headers}
