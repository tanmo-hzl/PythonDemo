...
Get Store Listing List - GET
    [Arguments]  ${variation}
    ${res}    Send Get Request    ${url-mik}    /mda/store/${seller_store_id}/listings    ${null}    ${seller_headers}
...
#    Set Suite Variable    ${sku_number}    ${sku_number}
    ...
    ${res}    Send Get Request    ${url-mik}    /mda/store/${seller_store_id}/listingInfo/${sku_number}   ${null}   ${seller_headers}
    [Return]  ${res.json()}
...
Get User Default Address - GET
    ...
    FOR  ${data}  IN  @{res}
        IF  '${data["defaultShippingAddress"]}'=='${True}'
            ${address_info}   Set Variable   ${data}
        END
    END
...
Flow From Search Listing To Submit Order
    ...
    Get Product Detail By Sku - POST   ${sku_number}
    Get Buyer Shipping Cart Id - GET
    Add Items To Buyer Shipping Cart - POST
    Get Buyer Shipping Cart Items List - GET
    Listing Checkout Initiate - POST
...
Buyer After Salers Chckorder
    ...
    ${res}      Send Get Request    ${url-mik}    ${mik-buyer-aftersale-checkorder}/${order_number}    ${param}    ${buyer_headers}
...
Buyer Return Order - POST
    ...
    ${body}    Get Buyer Return Order Body    ${buyer_order_detail}   ${can_return_order}  ${img_url}
...
Pre-initiate - POST
    [Arguments]   ${product_info_list}
    ${body}   get_pre_initiate_body   ${product_info_list}   ${Address_info}
    ${res}    Send Post Request    ${url-mik}    ${mik-checkout-pre-initiate}     ${body}    ${buyer_headers_post}
    Set suite Variable    ${Pre_initiate_res}   ${res.json()["preInitialSubOrderVoList"]}

Split Order And Initiate
     ${body}   get_split_order_and_initiate_body   ${Pre_initiate_res}   ${Address_info}   ${buyer_info_sign}
     ${res}    Send Post Request    ${url-mik}    ${mik-split-order-and-initiate}     ${body}    ${buyer_headers_post}
     Set suite Variable    ${initiate_res}    ${res.json()}

Submit Order - POST
    ${body}   get_submit_order_body   ${initiate_res}   ${wallet_bankcard}   ${Address_info}    ${buyer_info_sign}
    ${res}    Send Post Request    ${url-mik}    ${mik-checkout-submit-order}    ${body}    ${buyer_headers_post}
...
Export An Excel File Including Details Of All Orders - Get
    ${res}    Send Get Request    ${url-mik}    ${mik-seller-export-orders}    ${seller_headers_post}
