...
Send Seller Pre Application - POST
    ...
    Check Email Existed - POST
    ${body}    Get Seller Pre Application Body  ${url-mik}   ${random_id}
...
Update Store Profile - POST
    ...
    ${data}  Read File  upload-img-data
    IF   ${data}==None
...
Check Store isEA Seller - Get
    ...
    should be equal as strings    ${res.text}    {"code":"200","message":"Succeeded","data":true}
...
Get Store Onboarding Verify - Get
    ...
    Get Store Id By User Id - GET
    ${headers}    Set Get Headers - Seller
    ${param}   Set Variable   isCreate=true
    ${res}    Send Get Request    ${url-mik}    ${mik-store-onboarding-verify}/${seller_store_id}     ${param}    ${headers}
